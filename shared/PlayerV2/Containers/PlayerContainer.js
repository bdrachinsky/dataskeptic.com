...
class PlayerContainer extends Component {
    ...
    constructor(props) {
        ...
        this.isPodcast = this.isPodcast.bind(this)
        this.showPlayerOnBlog = this.showPlayerOnBlog.bind(this)
    ...
    isPodcast(){
        if(isEmpty(this.props.oepisode)){
            return false
        }
    ...
    showPlayerOnBlog(){
        if(!this.isPodcast()){
            this.props.dispatch({type:'PLAY_BLOG'})
        }
    }
    ...
    render() {
        const {player,mp3, pubDate,oepisode} = this.props;
        ...
        const howler = (
            <ReactHowler
                src={`https://s3.amazonaws.com/data-skeptic-bonus-feed/episodes/2017/prophecy-fulfillment.mp3`}
                html5={true}
                playing={is_playing}
...
export default connect(
    state => ({
        ...
        oepisode: state.player.getIn(['episode']).toJS()
    })
)(PlayerContainer)
